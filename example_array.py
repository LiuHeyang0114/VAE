...
for epoch in range(1, epoch_num + 1):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        if batch_idx == 0:
            nelem = data.size(0)
            nrow = 10
            save_image(data.view(nelem, 1, 28, 28), './images/image_array' + '.png', nrow=nrow)
...
with torch.no_grad():
    x = torch.arange(-5, 5, 10/8)
    y = torch.arange(-5, 5, 10/8)
    resultx, resulty = torch.meshgrid(x, y)
    z = torch.cat((resultx.reshape(64, 1), resulty.reshape(64, 1)), dim=1).float().cuda()
    sample = vae.decoder(z).cuda()

    save_image(sample.view(64, 1, 28, 28), './samples/sample_array' + '.png')